...
from dataLoader.ray_utils import patch_resize
...
from models.VGG import MultiDiscriminator, sliding_window
...
def render_test(args):
    ...
    if args.text is not None:
        style_name = args.text.replace(' ', '_')
    else:
    ...
    if args.render_train:
        ...
        evaluation_feature(train_dataset,tensorf, args, renderer, args.chunk_size, f'{logfolder}/{args.expname}/imgs_train_all/{style_name}',
                                N_vis=-1, N_samples=-1, white_bg = train_dataset.white_bg, ndc_ray=ndc_ray, style_img=style_img, device=device, args=args)
    ...
    if args.render_test:
        ...
        evaluation_feature(test_dataset,tensorf, args, renderer, args.chunk_size, f'{logfolder}/{args.expname}/imgs_test_all/{style_name}',
                                N_vis=-1, N_samples=-1, white_bg = test_dataset.white_bg, ndc_ray=ndc_ray, style_img=style_img, device=device, args=args)
...
def reconstruction(args):
    ...
    for iteration in pbar:
        ...
        feature_map, acc_map, style_feature, clip_loss, features_img, clip_feature_render = \
            renderer(rays_train, tensorf, chunk=args.chunk_size, N_samples=nSamples, white_bg = white_bg,
                                ndc_ray=ndc_ray, render_feature=True, style_img=style_img, device=device, is_train=True, args=args)
        
        feature_map = feature_map.reshape(patch_size, patch_size, 256)[None,...].permute(0,3,1,2)
        features_img = features_img.reshape(patch_size, patch_size, 256)[None,...].permute(0,3,1,2)
        clip_feature_render = clip_feature_render.reshape(patch_size, patch_size, 512)
        clip_feature_render = clip_feature_render / clip_feature_render.norm(dim=-1, keepdim=True)

        consistency_loss = nn.functional.mse_loss(feature_map, features_img.detach())

        # with torch.no_grad():
        rgb_map = tensorf.decoder(feature_map)
        rgb_map_img = tensorf.decoder(features_img)

        # *************************************************** #
        # Local train
        # window_size = 32
        # counter = torch.zeros(1, 512, patch_size, patch_size, device=device)
        # feature = torch.zeros(1, 512, patch_size, patch_size, device=device)

        # normalized_windows = []
        # for window, _, _ in sliding_window(rgbs_train[None, ...].permute(0,3,1,2), window_size, step=window_size // 2):
        #     normalized_windows.append(clip_normalize(window))

        # with torch.no_grad():
        #     window_features = tensorf.clip.clip_model.encode_image(torch.cat(normalized_windows, dim=0)).float().to(device)
        #     window_features = window_features / window_features.norm(dim=-1, keepdim=True)

        # for idx, (_, i, j) in enumerate(sliding_window(rgbs_train[None, ...].permute(0,3,1,2), window_size)):
        #     counter[:, :, i:i+window_size, j:j+window_size] += 1
        #     feature[:, :, i:i+window_size, j:j+window_size] += window_features[idx].view(1, 512, 1, 1)

        # normal_feature = (feature / counter).permute(0,2,3,1).squeeze(0)
        # *************************************************** #
        ...
        out_image_feature_img = tensorf.encoder(rgb_map_img)
        content_feature = tensorf.encoder(rgbs_train)

        # Local train
        # clip_feature_loss = nn.functional.l1_loss(normal_feature, clip_feature_render)
        ...
        if white_bg:
            ...
            content_loss_t = cal_mse_content_loss(torch.masked_select(content_feature.relu4_1, _mask),
                                                torch.masked_select(out_image_feature_img.relu4_1, _mask))
            # style loss
            style_loss = 0.
            style_loss_t = 0.
            for style_feature, image_feature, image_feature_1 in zip(style_feature, out_image_feature, out_image_feature_img):
                ...
                masked_img_feature_1 = torch.masked_select(image_feature_1, _mask).reshape(1,C,-1)
                style_loss += cal_adain_style_loss(style_feature, masked_img_feature)
                style_loss_t += cal_adain_style_loss(masked_img_feature_1, style_feature)

            content_loss *= args.content_weight
            content_loss_t *= args.content_weight
            style_loss *= args.style_weight
            style_loss_t *= args.style_weight

        else:
            # content loss
            content_loss = cal_mse_content_loss(content_feature.relu4_1, out_image_feature.relu4_1)
            content_loss_t = cal_mse_content_loss(content_feature.relu4_1, out_image_feature_img.relu4_1)
            # style loss
            style_loss = 0.
            style_loss_t = 0.
            for style_feature, image_feature, image_feature_1 in zip(style_feature, out_image_feature, out_image_feature_img):
                style_loss += cal_adain_style_loss(style_feature, image_feature)
                style_loss_t += cal_adain_style_loss(image_feature_1, style_feature)
        ...
        clip_loss *= args.style_weight

        # local train
        # total_loss = feature_tv_loss + image_tv_loss + content_loss + content_loss_t + style_loss + style_loss_t + clip_loss + consistency_loss + clip_feature_loss
        total_loss = feature_tv_loss + image_tv_loss + content_loss + content_loss_t + style_loss + style_loss_t + clip_loss + consistency_loss
        ...
        if iteration % args.progress_refresh_rate==0:
            summary_writer.add_scalar('train/c_loss', content_loss, global_step=iteration)
            summary_writer.add_scalar('train/s_loss', style_loss, global_step=iteration)
            ...
            summary_writer.add_scalar('train/clip_loss', clip_loss, global_step=iteration)
            summary_writer.add_scalar('train/consistency_loss', consistency_loss, global_step=iteration)
            summary_writer.add_scalar('train/s_loss_t', style_loss_t, global_step=iteration)
            summary_writer.add_scalar('train/c_loss_t', content_loss_t, global_step=iteration)
            # summary_writer.add_scalar('train/clip_feature_loss', clip_feature_loss, global_step=iteration)
            pbar.set_description(
                f'Step {iteration:05d}:'
                + f' c_loss = {content_loss.item():.2f}'
                + f' c_loss_t = {content_loss_t.item():.2f}'
                + f' s_loss = {style_loss.item():.2f}'
                + f' s_loss_t = {style_loss_t.item():.2f}'
                + f' clip_loss = {clip_loss.item():.2f}'
                # + f' clip_feature_loss = {clip_feature_loss.item():.2f}'
                + f' consistency_loss = {consistency_loss.item():.2f}'
            )
        ...
        if iteration % (args.progress_refresh_rate*20) == 0:
            summary_writer.add_image('output', make_grid([denormalize_vgg(rgbs_train).squeeze(), \
                                                ...
                                                denormalize_vgg(rgb_map_img).clamp(0, 1).squeeze(), \
                                                TF.resize(style_img, (patch_size,patch_size)).squeeze()],
                                                nrow=4, padding=0, normalize=False),
