...
from utils import *
from dataLoader.ray_utils import ndc_rays_blender, denormalize_vgg, normalize_vgg, clip_normalize
import clip


def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, 
                                render_feature=False, style_img=None, text=None, device='cuda', args=None):

    rgbs, alphas, depth_maps, weights, uncertainties = [], [], [], [], []
    features, accs, features_img = [], [], []
    clip_features = []
    s_mean_std_mat = None
    s_clip_mean_std_mat = None
    clip_loss = 0.
    # set text prompt
    text = args.text
    if style_img is not None:
        with torch.no_grad():
            ...
            if text is None:
                s_clip = tensorf.clip.clip_model.encode_image(clip_normalize(normalize_vgg(style_img)))
            else:
                s_clip = torch.from_numpy(tensorf.clip.clip_model.encode_text(
        ...
        s_clip_mean_std_mat = tensorf.stylizer.get_clip_mean_std_matrix(s_clip)
        if is_train:
            clip_loss = cal_clip_style_loss(s_clip_mean_std_mat, s_mean_std_mat)
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        if render_feature:
            feature_map, acc_map, feature_map_img, clip_feature = tensorf.render_feature_map(rays_chunk,
                                                                               s_mean_std_mat=s_mean_std_mat,
                                                                               s_clip_mean_std_mat=s_clip_mean_std_mat,
                                                                               is_train=is_train, ndc_ray=ndc_ray,
                                                                               N_samples=N_samples)
            features.append(feature_map)
            features_img.append(feature_map_img)
            accs.append(acc_map)
    ...
    if render_feature:
        if style_img is not None:
            return torch.cat(features), torch.cat(accs), style_feature, clip_loss, torch.cat(features_img), torch.cat(clip_features)
...
def evaluation_feature_path(test_dataset, tensorf, c2ws, renderer, chunk_size=2048, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, c2w in tqdm(enumerate(c2ws)):
        ...
        if style_img is None:
            ...
            feature_map, _, _, _, _, clip_feature = renderer(rays, tensorf, chunk=chunk_size, N_samples=N_samples, ndc_ray=ndc_ray,
                                white_bg = white_bg, render_feature=True, style_img=style_img, device=device)
        
        feature_map = feature_map.reshape(H, W, 256)[None,...].permute(0,3,1,2)
        clip_feature = clip_feature.reshape(H, W, 512)     
...
def cal_clip_style_loss(x, y):
    ...
    """
    x_mean, x_std, x_mat = x
    y_mean, y_std, y_mat = y

    return nn.functional.mse_loss(x_mean, y_mean.detach()) \
         + nn.functional.mse_loss(x_std, y_std.detach()) \
         + nn.functional.mse_loss(x_mat, y_mat.detach())
