...
from .VGG import Encoder, Decoder, UNetDecoder, PlainDecoder, CLIP
...
class TensorVMSplit(TensorBase):
    ...
    def change_to_style_mod(self, device='cuda'):
        ...
        self.clip = CLIP(device=device)

        self.stylizer = SimpleLinearStylizer(256).to(device)

        # Local train
        # self.clip_feature.requires_grad_(True)
    ...
    def get_optparam_groups_style_mod(self, lr_init_network, lr_finetune):
        grad_vars = [
                        ...
                        # Local train
                        # {'params': self.clip_feature.parameters(), 'lr': lr_finetune},
                    ]
        return grad_vars
    ...
    def render_feature_map(self, rays_chunk, s_mean_std_mat=None, s_clip_mean_std_mat=None, is_train=False, ndc_ray=False, N_samples=-1):
        clip_feature_map = None
        # sample points
        ...
        if s_mean_std_mat is not None:
            ...
            clip_features = torch.zeros((*xyz_sampled.shape[:2], 512), device=xyz_sampled.device)
        else:
            features = torch.zeros((*xyz_sampled.shape[:2], 256), device=xyz_sampled.device)  # 2048 300 256
        ...
        if app_mask.any():
            ...
            valid_clip_feature = self.clip_feature(valid_features)
            ...
            if s_mean_std_mat is not None:
                ...
                clip_features[app_mask] = valid_clip_feature

            features[app_mask] = valid_features

        # if s_mean_std_mat is not None:
        #     clip_feature_map = torch.sum(weight[..., None].detach() * clip_features, -2)
        
        # ******************************************************
        # text = 'background'
        # with torch.no_grad():
        #     s_clip = torch.from_numpy(self.clip.clip_model.encode_text(
        #             clip.tokenize([text], truncate=True).cuda()).float().data.cpu().numpy()).cuda()
        #     s_clip = s_clip / s_clip.norm(dim=-1, keepdim=True)
        #     output = (torch.nn.functional.cosine_similarity(clip_feature_map, s_clip, dim=-1) * 10) - 0.5
        #     # output = torch.where(output > 0.45, 1, 0)
        #     output = torch.where(output > 0.49, 0, 1)
        # ******************************************************    

        feature_map = torch.sum(weight[..., None] * features, -2)
        feature_map_img = feature_map
        acc_map = torch.sum(weight, -1)

        # ******************************************************   
        # feature_map_res = feature_map.clone()
        # feature_map = feature_map * output[..., None]
        # feature_map_res = feature_map_res * (1 - output[..., None])
        # ******************************************************   

        # style transfer on 2d
        if s_mean_std_mat is not None:
            feature_map_img = self.stylizer.transfer_style_2D(s_mean_std_mat, feature_map_img.transpose(0,1)[None,...], acc_map)
            feature_map_img = feature_map_img.squeeze().transpose(0,1)
            feature_map = self.stylizer.transfer_style_2D(s_clip_mean_std_mat, feature_map.transpose(0,1)[None,...], acc_map)
            feature_map = feature_map.squeeze().transpose(0,1)
            
            # ******************************************************   
            # feature_map_res = self.stylizer.transfer_style_2D(s_mean_std_mat, feature_map_res.transpose(0,1)[None,...], acc_map)
            # feature_map_res = feature_map_res.squeeze().transpose(0,1)
            # ******************************************************   
        
        # ******************************************************   
        # feature_map =  torch.where(output[..., None].repeat(1, 256).bool(), feature_map, feature_map_res)
        # ******************************************************   
        
        return feature_map, acc_map, feature_map_img, clip_feature_map
