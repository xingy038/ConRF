...
class Render_CLIP_feature(torch.nn.Module):
    def __init__(self,inChanel, feape=6, featureC=128):
        super(Render_CLIP_feature, self).__init__()

        self.in_mlpC = 2*feape*inChanel + inChanel #
        self.feape = feape
        layer1 = torch.nn.Linear(self.in_mlpC, featureC)
        layer2 = torch.nn.Linear(featureC, featureC)
        layer3 = torch.nn.Linear(featureC, 512)
    ...
    def forward(self, features):
        ...
        if self.feape > 0:
            indata += [positional_encoding(features, self.feape)]
        mlp_in = torch.cat(indata, dim=-1)
        CLIP_feature = self.mlp(mlp_in)
        CLIP_feature = torch.sigmoid(CLIP_feature)
...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, device, density_n_comp = 8, appearance_n_comp = 24, app_dim = 27,
        ...
        self.clip_feature = Render_CLIP_feature(256, fea_pe, featureC).to(device)
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        rgb = torch.zeros((*xyz_sampled.shape[:2], 3), device=xyz_sampled.device) # 4096 300 3
        ...
        if app_mask.any():
            ...
            rgb[app_mask] = valid_rgbs # 4096 300 3
